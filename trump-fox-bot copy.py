...
def readDatabase(my_bucket): 
    #my_bucket.download_file('trump-fox-bot', 'tweets.json', 'local.json')
...
def analyze(tweet, chyrons, api, stopwords):  
    ...
    for chyron in chyrons:
        clean_chyron = remove_stopwords(chyron[5], stopwords) 
        if (fuzz.token_set_ratio(clean_tweet, clean_chyron) > 50):
            api.update_status(checkLength(chyron[5], replaceForEST(chyron[0],chyron[1]), tweet[1]))
...
def get_chyrons():
    ...
    response = http.request('GET', 'https://archive.org/services/third-eye.php?last=0.75')
